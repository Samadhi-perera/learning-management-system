<x-lms-layout title="Create Course" header="Add New Academic Course">
    <div class="max-w-3xl">
            ...
            <form action="{{ route('admin.courses.store') }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-3 gap-4">
                    <div>
                        ...
                        <input type="text" name="code" value="{{ old('code') }}" placeholder="e.g. CS101" required class="w-full rounded-xl border-slate-200 text-sm px-4 py-2.5 focus:ring-indigo-500 focus:border-indigo-500">
                    </div>
                    <div class="sm:col-span-2">
                        <label class="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">Course Title</label>
                        <input type="text" name="title" value="{{ old('title') }}" placeholder="e.g. Introduction to Software Engineering" required class="w-full rounded-xl border-slate-200 text-sm px-4 py-2.5 focus:ring-indigo-500 focus:border-indigo-500">
                    </div>
                </div>
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <input type="number" name="credits" value="{{ old('credits', 3) }}" min="1" max="10" required class="w-full rounded-xl border-slate-200 text-sm px-4 py-2.5 focus:ring-indigo-500 focus:border-indigo-500">
                    </div>
                </div>

                <div>
                    <label class="block text-xs font-semibold text-slate-700 uppercase tracking-wider mb-1">Course Description & Objectives</label>
                    <textarea name="description" rows="4" placeholder="Overview of the course topics, learning outcomes, and expectations..." class="w-full rounded-xl border-slate-200 text-sm px-4 py-2.5 focus:ring-indigo-500 focus:border-indigo-500">{{ old('description') }}</textarea>
                </div>

                <div class="flex items-center gap-2">
                    <input type="checkbox" id="is_active" name="is_active" value="1" {{ old('is_active', true) ? 'checked' : '' }} class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500">
